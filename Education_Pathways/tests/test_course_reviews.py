...
def test_add_review(course_code, user_name, review, expected_code):
    ...
    if user_name is None and review is None:
        resp = tester.post(f"/course/reviews?course={course_code}")    
    else:
        resp = tester.post(f"/course/reviews?course={course_code}&user_name={user_name}&review={review}")
    assert resp.status_code == expected_code

@pytest.mark.parametrize("course_code", ["TESTCOURSE"])
def test_get_review(course_code):
    tester = app.test_client()
    resp = tester.get(f"/course/reviews?course={course_code}")
