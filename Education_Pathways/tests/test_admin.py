...
def test_update_info(code, name, desc, prereq, excl, coreq, department, div):
    ...
    resp = tester.post(
        f"/course/updateInfo?course_code={code}&course_name={name}&description={desc}&prereq={prereq}&exclusions={excl}&coreq={coreq}&department={department}&division={div}&idCode={code}")
...
def test_update_bad_info(code, name, desc, prereq, excl, coreq, department, div, idCode):
    tester = app.test_client()
    resp = tester.post(
        f"/course/updateInfo?course_code={code}&course_name={name}&description={desc}&prereq={prereq}&exclusions={excl}&coreq={coreq}&department={department}&division={div}&idCode={idCode}")
    assert resp.status_code == 200
    resp_data = json.loads(resp.data.decode('utf-8'))
    # Expect an error msg
